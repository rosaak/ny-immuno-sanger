...
@app.command()
def message():
    typer.echo(f"This script matches the abi file from a single dir with the probe file and move it to a different dir")

@app.command()
def single_dir_analysis(config_file_path:str):
    ### START LOGGING 
    Path('logs').mkdir(parents=True, exist_ok=True)
    log_fp = "logs/single_log"
    log = utils.make_logger(log_fp)
    log.info("START")
    ...
    try:
        ...
        if not vx_gb_abi_match_filtered.empty:
            ...
            log.info(f"[+] INFO: There are no matched GB - {abbr} aligned datafrme `vx_gb_abi_match_filtered`")
        print(f"[+] Check {log_fp} for logs")
        print(f"[+] Check {results_dir} for results")
    except:
        print(f"[-] Check {log_fp} for errors / abort")
